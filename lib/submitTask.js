...
let submitTask = () => {
	...
	if(os.platform() === 'win32')
		solutionFile = __dirname + exercises[userDataJSON.taskCount].op;
	else
		solutionFile = __dirname + '/workspace/' + exercises[userDataJSON.taskCount].op;
	...
	PythonShell.run(submittedFile, null, (err, result) => {
		...
		PythonShell.run(solutionFile, null, (err, sol) => {
			...
			if(result === undefined || sol === undefined){
					console.log(chalk.red(` Kindly have a look at task${userDataJSON.taskCount}.py`));
					process.exit(1);
			}
