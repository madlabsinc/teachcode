...
let createRepo = () => {
	...
	.then((info) => {
			shell.exec(`git init && curl --silent --output /dev/null -u ${info.username} ${API_URL} -d '{"name": "Teach-Code-solutions"}'`, (err) => {
				if (err) throw err;
				repoUrl = `https://www.github.com/${info.username}/Teach-Code-solutions`;
