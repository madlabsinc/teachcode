...
let checkSolution = (submittedFileContent, solutionFileContent) => {
	try {
		if (submittedFileContent.toString() === solutionFileContent.toString()) {
			...
			if (userDataJSON.taskCount === exercises.length) {
				...
				console.log(chalk.cyanBright(' Info: ') + chalk.yellowBright('No more tasks available!'));
				process.exit(1);
			}
			...
			console.log(chalk.red(`\n There's something wrong with the file task${userDataJSON.taskCount + 1}.${fileExtension}\n`));
			console.log('\n' + err);
	}
};
...
exports.submitTask = () => {
	...
	if (learningTrack === 'Python') {
		...
		solutionFile = __dirname + '/../workspace/python' + exercises[userDataJSON.taskCount].op;
		fileExtension = 'py';
	} else {
			exercises = require('../workspace/js/tasks');
			solutionFile = __dirname + '/../workspace/js' + exercises[userDataJSON.taskCount].op;
			fileExtension = 'js';
		}

	if (userDataJSON.taskCount !== exercises.length) {
		console.log(chalk.green(`\nUser: ${userDataJSON.username}\t\t\t\t\t\tProgress: ${userDataJSON.taskCount + 1}/${exercises.length}`));
	} else {
	...
	if (submittedFileContent.length === 0) {
			console.log(chalk.red(`\n Solution file task${userDataJSON.taskCount + 1}.${fileExtension} is empty!\n`));
			process.exit(1);
	}
		...
		PythonShell.run(submittedFile, null, (err, result) => {
			...
			PythonShell.run(solutionFile, null, (err, solution) => {
				...
				if (typeof result === 'undefined' || typeof solution === 'undefined') {
						console.log(chalk.red(`\n Kindly have a look at task${userDataJSON.taskCount}.${fileExtension}`));
						process.exit(1);
				}
